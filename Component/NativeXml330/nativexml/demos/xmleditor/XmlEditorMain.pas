{ Unit XmlEditorMain
  ...
  an incompatible parameter list (this is due to UnicodeString vs WideString)
    ...
    Properties/methods of TNativeXmlEx compared to TNativeXml
...
uses
  ...
  NativeXml, sdDebug,
  SynEdit, SynMemo,
  SynEditHighlighter, SynHighlighterXML, sdXmlOutputOptionsDlg;
...
const
  ...
  cDefaultStyleSheet = 'href="mystylesheet.xsl"';
...
procedure DownloadURLStream(const Url: Utf8String; Dest: TStream);
var
  NetHandle: HINTERNET;
  UrlHandle: HINTERNET;
...
begin
  if not assigned(Dest) then
    exit;
  Dest.Size := 0;
  NetHandle := InternetOpenA('Delphi', INTERNET_OPEN_TYPE_PRECONFIG, nil, nil, 0);
...
var
  S: UTF8String;
begin
  S := FXml.StyleSheet.Value;
  if Length(S) = 0 then
    S := cDefaultStyleSheet;
  S := InputBox('Add stylesheet string', 'Stylesheet string:', S);
  FXml.StyleSheet.Value := Format('href="%s"', [S]);
end;
...
begin
  ...
  begin
    ...
    try
      ...
      // not applicable with new NativeXml - must make a canonical xml
      ...
      if Length(FXml.EncodingString) > 0 then
        sbMain.SimpleText := sbMain.SimpleText +
          Format(' Encoding="%s"', [FXml.EncodingString]);
      sbMain.SimpleText := sbMain.SimpleText +
        Format('unique strings: %d', [FXml.StringTable.StringCount]);
    except
...
begin
  ...
  try
    ...
    M.SaveToFile('temp_url_stream.txt');
    // Load from this stream
    M.Position := 0;
    FXml.LoadFromStream(M);
    // Regenerate views
    Regenerate;
{    M.Position := 0;
    M.SaveToFile('temp_url_stream.txt');}
  ...
  try
    ...
    begin
      ...
      begin
        FUseStoredEncoding := False;
        FExternalEncoding := seAnsi;
        FExternalCodepage := sdEncodingStringToCodepage(Dlg.edCodepage.Text);
      end;
      ...
      begin
        ...
        begin
          ...
          FXml.EncodingString := sdCodepageToEncoding(FExternalCodepage);
        end;

        // Other options
        if Dlg.rgXmlFormat.ItemIndex = 0 then
...
begin
  ...
  xfCompact:  FXml.XmlFormat := xfReadable;
  end;
  UpdateMenu;
end;
...
begin
  ...
  begin
    ...
    1: CellText := FData^.FNode.ValueUnicode;
    end;
  end;
end;
...
var
  OldFormat: TsdXmlFormatType;
begin
  // Show our Xml Document in readable format
  Oldformat := FXml.XmlFormat;
  try
    FXml.IndentString := '  ';
